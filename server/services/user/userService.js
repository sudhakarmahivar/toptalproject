...
class UserService {
  ...
  /**
   * Creates new user. This could be self registration ( when anonymous) or other user registrations by admin/manager roles
   * @async
  ...
  /**
   * Update user record for self, others when authorized
   * @async
   * @param {UserModel} userModel
   */
  ...
  /**
   * Authenticate user.
   * On successfull authentiction, return access token along with use model
   * @async
   * @param {loginModel} loginModel userName,password field expected
   */
  ...
  /**
   * Invalidates the token for further use
   * @async
   * @param {string} token
   */
  async logout(token) {
